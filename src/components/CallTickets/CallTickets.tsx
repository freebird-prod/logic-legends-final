...
import { AlertTriangle, Clock, User, CheckCircle, ArrowUp, Users, Eye } from 'lucide-react';
...
export const Escalations: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = TicketService.listenToTickets((allTickets) => {
            // Filter for ALL call-based tickets
            const callTickets = allTickets.filter((ticket: Ticket) =>
                ticket.source === 'call' &&
                (ticket.status === 'open' || ticket.status === 'in_progress')
            );
            setEscalatedTickets(callTickets);
            setLoading(false);
        });
    ...
    const getTicketType = (ticket: Ticket) => {
        ...
        if (ticket.sentiment === 'neutral') return 'Neutral Call';
        if (ticket.sentiment === 'positive') return 'Happy Caller';
        return 'Call Ticket';
    };
    ...
    const getSeverityColor = (ticket: Ticket) => {
        ...
        if (ticket.sentiment === 'neutral') return 'bg-yellow-100 text-yellow-800 border-yellow-200';
        if (ticket.sentiment === 'positive') return 'bg-green-100 text-green-800 border-green-200';
    ...
    return (
        <div className="space-y-6">
            <div>
                <h2 className="text-2xl font-bold text-gray-900 mb-2">Call Tickets 📞</h2>
                <p className="text-gray-600">
                    All active call-based customer tickets requiring attention.
                </p>
            </div>
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <div className="text-sm text-gray-600">Active Call Tickets</div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <div className="text-2xl font-bold text-gray-900">
                                {escalatedTickets.filter(t => t.priority === 'priority').length}
                            </div>
                            <div className="text-sm text-gray-600">Priority Tickets</div>
                        </div>
                    </div>
                </div>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200">
                <div className="p-6 border-b border-gray-200">
                    <h3 className="text-lg font-semibold text-gray-900">All Call Tickets</h3>
                </div>
                <div className="divide-y divide-gray-200">
                    {escalatedTickets.length === 0 ? (
                        <div className="p-6 text-center text-gray-500">
                            <AlertTriangle className="h-12 w-12 mx-auto mb-4 text-gray-300" />
                            <p>No active call tickets at this time. Great job! 🎉</p>
                        </div>
                    ) : (
